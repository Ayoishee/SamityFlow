...
public class LoanApplicationRepository {
    ...
    public void update(LoanApplication application) {
        ...
        try(Connection c = connection != null ? connection : database.connect();PreparedStatement s=c.prepareStatement(sql)){
    ...
    public Optional<LoanApplication> findById(int id) {
        try(Connection c = connection != null ? connection : database.connect();PreparedStatement s=c.prepareStatement("SELECT * FROM loan_applications WHERE id=?")){s.setInt(1,id);try(ResultSet r=s.executeQuery()){return r.next()?Optional.of(map(r)):Optional.empty();}}
    ...
    public List<LoanApplication> findAll() {
        ...
        try(Connection c = connection != null ? connection : database.connect();Statement s=c.createStatement();ResultSet r=s.executeQuery("SELECT * FROM loan_applications ORDER BY id DESC")){while(r.next())items.add(map(r));return items;}
